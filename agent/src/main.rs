...
*/
async fn send_udp_request<T>(
    message: <T as Plugin>::PluginMessage,
    target: &str,
    port: u16,
) -> utilities::core::Result<Vec<u8>>
where
    T: Plugin,
{
    // Attempt to downcast the plugin message to the plugin message of the plugin type.
    let cast_message = utilities::core::downcast_plugin_message::<_, T::PluginMessage>(&message)?;
...
*/
async fn runtime<T>(plugin: T) -> utilities::core::Result<()>
where
    T: Plugin,
    <T as Plugin>::PluginMessage: std::fmt::Display + Clone,
{
    ...
    loop {
        ...
        let heartbeat_downcast =
            utilities::core::downcast_plugin_message::<_, T::PluginMessage>(&heartbeat_generic)?;

        // Send the downcasted heartbeat as a UDP request and get the server message bytes back.
        let server_message_bytes =
            send_udp_request::<T>(heartbeat_downcast.clone(), target, port).await?;
        ...
        'chain: loop {
            ...
            if let Some(client_message_generic) =
                T::agent_runtime(&plugin, server_message_downcast)?
            {
                // Downcast the response packet into the plugin message type, using the AsAny downcaster.
                let client_message_downcast = utilities::core::downcast_plugin_message::<
                    _,
                    T::PluginMessage,
                >(&client_message_generic)?;

                // Send the downcasted heartbeat as a UDP request and get the server message bytes back.
                let server_message_bytes =
                    send_udp_request::<T>(client_message_downcast.clone(), target, port).await?;
